...
- (void)layoutSubviews {
    if(_isNeedRefreshLayout) {
        ...
        for(NSInteger i = 0; i < _tabItems.count; i++) {
            DHSegementLabel *tabItem = (DHSegementLabel *)_tabItems[i];
...
- (void)initBaseSettings {
    ...
    _isChangeByClick = NO;
    _bodyBounces = YES;
    _titleStyle = DHSegmentTitleStyleDefault;
    _indicatorStyle = DHSegemntTabIndicatorStyleDefault;
    _minScale = kMinScale;
    _selectedColorR = 1;
    _selectedColorG = 0;
    _selectedColorB = 0;
...
- (void)initTabView {
    ...
    for(NSInteger i = 0; i < _numberOfTabItems; i++) {
        DHSegementLabel *tabItem = [[DHSegementLabel alloc] init];
...
- (void)reviseTabContentOffsetBySelectedIndex:(BOOL)isAnimate {
    DHSegementLabel *currentTabItem = _tabItems[_selectedTabIndex];
...
- (void)finishReviseTabContentOffset {
    ...
    _isNeedRefreshLayout = YES;
    _isChangeByClick = NO;
    if([self.delegate respondsToSelector:@selector(pageSegmentViewDidEndChange)]) {
        if(_lastSelectedTabIndex != _selectedTabIndex) {
            [self.delegate pageSegmentViewDidEndChange];
        }
    }
...
- (void)changeSelectedItemToNextItem:(NSInteger)nextIndex {
    DHSegementLabel *currentTabItem = _tabItems[_selectedTabIndex];
    DHSegementLabel *nextTabItem = _tabItems[nextIndex];
...
- (void)resetTabItemScale {
    for(NSInteger i = 0; i < _numberOfTabItems; i++) {
        DHSegementLabel *tabItem = _tabItems[i];
...
- (void)addIndicatorViewWithStyle {
    switch (_indicatorStyle) {
        case DHSegemntTabIndicatorStyleDefault:
        case DHSegemntTabIndicatorStyleFollowText:
        case DHSegemntTabIndicatorStyleStretch:
...
- (void)layoutIndicatorView {
    ...
    DHSegementLabel *selecedTabItem = _tabItems[_selectedTabIndex];
...
- (void)changeChildControllerOnClick:(UITapGestureRecognizer *)tap {
    ...
    if(nextIndex != _selectedTabIndex) {
        if(_titleStyle == DHSegmentTitleStyleDefault) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if(scrollView == self.tabView) {
        ...
        //调整title
        switch (_titleStyle) {
            case DHSegmentTitleStyleDefault:
                [self changeTitleWithDefault];
                break;
            case DHSegmentTitleStyleGradient:
                [self changeTitleWithGradient];
                break;
            case DHSegmentTitleStyleBlend:
...
- (void)changeTitleWithGradient {
    if(_leftItemIndex != _rightItemIndex) {
        ...
        DHSegementLabel *leftTabItem = _tabItems[_leftItemIndex];
        DHSegementLabel *rightTabItem = _tabItems[_rightItemIndex];
...
- (CGFloat)getIndicatorWidthWithTitle:(NSString *)title {
    if(_indicatorStyle == DHSegemntTabIndicatorStyleDefault || _indicatorStyle == DHSegemntTabIndicatorStyleStretch) {
...
- (void)setSelectedTabIndex:(NSInteger)selectedTabIndex {
    if(selectedTabIndex >= 0 && selectedTabIndex < _numberOfTabItems && _selectedTabIndex != selectedTabIndex) {
        ...
        if(_titleStyle == DHSegmentTitleStyleGradient) {
...
- (void)setSelectedColor:(UIColor *)selectedColor {
    ...
    DHSegementLabel *tabItem = _tabItems[_selectedTabIndex];
...
- (void)setTitleStyle:(DHSegmentTitleStyle)titleStyle {
    if(_titleStyle == DHSegmentTitleStyleDefault) {
...
- (void)setIndicatorStyle:(DHSegemntTabIndicatorStyle)indicatorStyle {
    if(_indicatorStyle == DHSegemntTabIndicatorStyleDefault) {
